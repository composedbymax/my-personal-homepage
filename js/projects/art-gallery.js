...
const floorTexture = new THREE.TextureLoader().load('marble.jpeg');
const ceilingTexture = new THREE.TextureLoader().load('marble.jpeg');
...
// Create front and back walls with the same texture
const frontBackWallGeometry = new THREE.BoxGeometry(tunnelWidth, tunnelHeight, 0.1);
const frontWall = new THREE.Mesh(frontBackWallGeometry, wallMaterial);
frontWall.position.z = -tunnelLength / 2;
scene.add(frontWall);

const backWall = new THREE.Mesh(frontBackWallGeometry, wallMaterial);
backWall.position.z = tunnelLength / 2;
scene.add(backWall);
